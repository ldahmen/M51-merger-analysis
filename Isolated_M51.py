...
"""This program shows an isolated evolution of galaxy M51."""
...
#--- the data in a unique way, and the number of stars you'd like to populate
#--- the simulation. It uses the functions that Jose has created.
def isolated_evolution(ts,N):
    N = N
    v = 0
    velocity_tag = "ISO_N" + str(N)
    #---- Galaxy Parameters
    ...
    #.how much time do you want to create rings for
    t = time_step
    distances = []
...
################################################################################
    ...
    def plot_2D(field = 'video', save_frame=False, eps_tag=False):
        ...
        for i in np.arange(time_step):
            ...
            ax.set_title('Isolated $M51$ Evolution' )

            ax.set_xlim(-60,60) # delete these if you like your axes to move
            ax.set_ylim(-50, 50) # ^^       ^^      ^^      ^^      ^^
            ax.legend((stars, spir, green_circle, purple_circle), ('$Stars$', '$M51$ $Center$', '$80\%$ $Radius$','$20\%$ $Radius$')
            ...
            if save_frame == False:
                ...
                #if not os.path.exists('./final_plot2d2_0.2'): os.mkdir('./final_plots2d2_0.2')
                ...
                #plt.savefig('./final_plots2d2_0.2/'+save_tag+'_'+file_type)
